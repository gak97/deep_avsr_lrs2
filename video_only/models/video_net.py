...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, dModel, maxLen):
        ...
        position_even = position / div_term.unsqueeze(0)
        position_odd = position / div_term[:dModel//2].unsqueeze(0)
        pe[:, 0::2] = torch.sin(position_even)
        pe[:, 1::2] = torch.cos(position_odd)
...
class VideoNet(nn.Module):
    ...
    def __init__(self, dModel, nHeads, numLayers, peMaxLen, fcHiddenSize, dropout, numClasses):
        ...
        # self.positionalEncoding = PositionalEncoding(dModel=7, maxLen=peMaxLen)
